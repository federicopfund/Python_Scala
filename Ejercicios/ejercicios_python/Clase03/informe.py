...
def mostrar_pantalla():
    line()
    print(f'| Nombre   | N_Cajones | Precio_Compra | precio_venta  |')
    line()
    n_filas=0
    for n_filas, e in enumerate(precio_compra):
        print(f'| {precio_compra[n_filas]["nombre"]:<9} | {precio_compra[n_filas]["cajones"]:^8} | {precio_compra[n_filas]["precio"]:^13} | {precio_venta[precio_compra[n_filas]["nombre"]]:^13} |' )
    ...
    line1()
    print(f'|   {total_pagado_en_compra:<11}  |   {recaudacion_de_ventas:^12}|   {diferencia_en_venta:^18}|')
    line1()
